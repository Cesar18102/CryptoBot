    ...
    public class ServerCommunicator : IServerCommunicator
    {
        ...
        public async Task<IServerResponse> SendQuery(IQuery query, CookieContainer container = null)
        {
            ...
            try
            {
                ...
                else if (query.Method == QueryMethod.DELETE)
                    response = await QueryDELETE(queryUrl, query, container);
        ...
        private async Task<(string, Dictionary<string, string>, CookieContainer)> QueryDELETE(string url, IQuery query, CookieContainer container)
        {
